...
FString ToStr( UObject* p, UBOOL bShort )
{
	...
	s += SWF_SEP + FString::Printf(TEXT("%s"),p->GetFullName());
...
FString ToStr( const FSwSoundId p, UBOOL bShort )
{
	if( bShort )
		...
		return FString::Printf( TEXT("%u:%u:%u:%s"), p.Id, int(p.GetSlot()), int(p.GetNoOverride()), *ToStr(p.GetActor()) );
}
...
FString FMOD_SPEAKERMODEToStr( const FMOD_SPEAKERMODE p, UBOOL bShort )
{
	...
	case FMOD_SPEAKERMODE_PROLOGIC:		s += FString(appSpc(1)) + TEXT("PROLOGIC"); break;
