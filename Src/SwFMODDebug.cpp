...
void USwFMOD::DumpPlugins()
{
	...
	{
		SWF_FMOD_CALL( System->getPluginInfo(FMOD_PLUGINTYPE_OUTPUT, i, pname, 1024, pver.GetRaw() ) );
		SWF_LOG( SWF_NAME, TEXT("Output Plugin[%d]: %s v%s"), i, *ToStr(appFromAnsi(pname)), *ToStr(pver) );
	}
	...
	for( i=0; i!=numplugins; ++i )
	{
		SWF_FMOD_CALL( System->getPluginInfo(FMOD_PLUGINTYPE_CODEC, i, pname, 1024, pver.GetRaw() ) );
		SWF_LOG( SWF_NAME, TEXT("Codec Plugin[%d]: %s v%s"), i, *ToStr(appFromAnsi(pname)), *ToStr(pver) );
	}
	...
	for( i=0; i!=numplugins; ++i )
	{
		SWF_FMOD_CALL( System->getPluginInfo(FMOD_PLUGINTYPE_DSP, i, pname, 1024, pver.GetRaw() ) );
		SWF_LOG( SWF_NAME, TEXT("DSP Plugin[%d]: %s v%s"), i, *ToStr(appFromAnsi(pname)), *ToStr(pver) );
	}
